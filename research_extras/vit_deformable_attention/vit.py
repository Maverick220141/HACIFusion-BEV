...
from ..utils import my_load_checkpoint as load_checkpoint
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., with_cp=False,
        ...
        # NOTE: drop path for stochastic(随机) depth, we shall see if this is better than dropout here
    ...
    def forward(self, x, H, W):
        ...
        #     checkpoint检查点，不保留中间变量，可以增大batch，用时间换显存
        else:
            x = _inner_forward(x)
...
class TIMMVisionTransformer(BaseModule):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, residual_indices=[], embed_dim=768,
        ...
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]  # stochastic depth decay rule 随机深度衰减规则
